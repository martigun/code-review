...
struct Node *_addNode(struct Node *cur, TYPE val){
	...
	//if new val is less than current val
...
int containsBSTree(struct BSTree *tree, TYPE val){
	...
	//make sure that tree and val exist
	assert(tree != 0);
	assert(val != 0);

	//create cur pointer to the root
	struct Node * cur = tree->root;

	//while the values are not equal...
	while (compare(val, cur->val) != 0){

		//if the value is less than cur
		if (compare(val, cur->val) == -1){

			//return 0 if the left is null
			if (cur->left == 0) return 0;

			//set cur to left if it exits
			cur = cur->left;
		}

		//if the value is greater than cur
		if (compare(val, cur->val) == 1) {

			//return 0 if the right is null
			if (cur->right == 0) return 0;

			//set cur to right if it exits
			cur = cur->right;
		}
	}
...
TYPE _leftMost(struct Node *cur){
	...
	//create temp ptr pointing to cur
	struct Node * ptr = cur;

	//while an left node exists, move pointer left
...
struct Node *_removeLeftMost(struct Node *cur){
	...
	//make sure that cur is not null
	assert(cur != 0);

	//if there is nothing to the left
	if (cur->left == 0){

		//if there is something to the right, return right
		if (cur->right != 0) return cur->right;

		//if there is no right child...
		//free the node
		free(cur);

		//return null, since this no longer points to anything
		return 0;

	} else {

		//else, run again on next left value
		cur->left = _removeLeftMost(cur->left);
	}

	//return cur to build back up
	return cur;
...
struct Node *_removeNode(struct Node *cur, TYPE val){
	...
	//base case- if value is found
	if (compare(val, cur->val) == 0){

		//if right child is null, return left child (even if left child is NULL)
		if (cur->right == 0) return cur->left;

		//if there is a right child...
		if (cur->right != 0){

			//set the current value to the leftmost value of the right child
	...
	//recursive cases - if val is LT or GT the cur val
	if (compare(val, cur->val) == -1) cur->left = _removeNode(cur->left,val);
	if (compare(val, cur->val) == 1) cur->right = _removeNode(cur->right,val);
...
int main(int argc, char *argv[]){	
	...
	//After implementing your code, please uncommnet the 
	//following calls to the test functions and test your code 
